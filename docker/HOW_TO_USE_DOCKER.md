...
Use the Dockerized version of HiveLink for a much faster and complete startup of HiveLink.

### Minimum Requirements

> [!TIP]
> Running HiveLink on AWS/GCP/Azure?
...
\*HiveLink by default uses a built-in vector database powered by [LanceDB](https://github.com/lancedb/lancedb)

\*HiveLink by default embeds text on instance privately [Learn More](../server/storage/models/README.md)

## Recommend way to run dockerized HiveLink!
...
> the docker container using HiveLink as `localhost:xxxx` will not resolve for the host system.
>
...
```shell
docker pull mintplexlabs/HiveLink
```

<table>
<tr>
<th colspan="2">Mount the storage locally and run HiveLink in Docker</th>
</tr>
<tr>
<td>
  Linux/MacOs
...
```shell
export STORAGE_LOCATION=$HOME/HiveLink && \
...
$env:STORAGE_LOCATION="$HOME\Documents\HiveLink"; `
...
services:
  HiveLink:
    image: mintplexlabs/HiveLink
    container_name: HiveLink
    ports:
    - "3001:3001"
    cap_add:
      - SYS_ADMIN
...
Go to `http://localhost:3001` and you are now using HiveLink! All your data and progress will persist between
...
- `touch server/storage/HiveLink.db` to create empty SQLite DB file.
- `cd docker/`
...
The integrations below are templates or tooling built by the community to make running the docker experience of HiveLink easier.

### Use the Midori AI Subsystem to Manage HiveLink

Follow the setup found on [Midori AI Subsystem Site](https://io.midori-ai.xyz/subsystem/manager/) for your host OS
After setting that up install the HiveLink docker backend to the Midori AI Subsystem.
...
Then in docker you need to replace that localhost part with `host.docker.internal`. For example, if running Ollama on the host machine, bound to http://127.0.0.1:11434 you should put `http://host.docker.internal:11434` into the connection URL in HiveLink.
